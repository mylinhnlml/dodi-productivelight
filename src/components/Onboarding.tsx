...
function SlideContent({ slide, keyId, direction, signIn, loading }: { slide: SlideKey; keyId: string; direction: 1 | -1; signIn: (p: "google" | "apple") => void; loading: "google" | "apple" | null }) {
  ...
  return (
    <div key={keyId} className={`absolute inset-0 ${enterAnim}`}>
      ...
      {slide === "cta" && <CTASlide signIn={signIn} loading={loading} />}
    </div>
  );
}
...
function CTASlide({ signIn, loading }: { signIn: (p: "google" | "apple") => void; loading: "google" | "apple" | null }) {
  const [checked, setChecked] = useState(false);

  const handleTick = () => {
    if (checked || loading) return;
    setChecked(true);
    // Small delay so the tick animation is felt before redirect
    setTimeout(() => signIn("google"), 450);
  };

  return (
    <div
      className="w-full h-full flex flex-col items-center justify-center px-6 pb-32"
      style={{ background: "linear-gradient(160deg, #FFF3DC 0%, #FFE4A0 100%)" }}
    >
      <div className="text-[64px] leading-none" style={{ animation: "pulse-soft 2.5s ease-in-out infinite" }}>☀️</div>
      <h1 className="font-extrabold text-center mt-5" style={{ fontSize: 24, color: "#1A1A1A", maxWidth: 280 }}>
        Finish your first gentle task
      </h1>
      <p className="text-center mt-3" style={{ fontSize: 13, color: "#888780", maxWidth: 260, lineHeight: 1.7 }}>
        Tap the circle to complete it — we'll sign you in and your softest era begins.
      </p>
      ...
      <button
        ...
        className="mt-8 w-full max-w-xs rounded-2xl bg-white shadow-lg p-4 flex items-center gap-3 text-left transition-transform active:scale-[0.98] disabled:cursor-default"
      ...
      >
        <span
          ...
          style={{
            width: 32,
            height: 32,
            borderColor: checked ? "hsl(45 95% 58%)" : "#E5DCC7",
            background: checked ? "hsl(45 95% 58%)" : "transparent",
          }}
        >
          {checked && (
        ...
        <div className="flex-1">
          <div className="font-extrabold text-sm text-stone-800">Start my gentle journey</div>
          <div className="text-[11px] text-stone-500">
            {loading === "google" ? "Signing in with Google…" : checked ? "Beautiful ✨" : "Tap to complete · 1 task"}
          </div>
        </div>
      ...
      <p className="mt-4 text-[11px] text-stone-500">
        By continuing you'll sign in with Google · Free to start
      </p>
    </div>
  );
}

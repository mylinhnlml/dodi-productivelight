CREATE OR REPLACE FUNCTION public.get_redemption_rank(_user_id uuid)
...
AS $$
  ...
  me AS (
    SELECT COALESCE((SELECT cnt FROM counts WHERE user_id = _user_id), 0)::bigint AS cnt
  )
  SELECT
...
GRANT EXECUTE ON FUNCTION public.get_redemption_rank(uuid) TO authenticated;